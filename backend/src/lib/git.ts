...
export async function commitAndPush(slug: string, message: string): Promise<void> {
  ...
  const relativeExists = (relPath: string) => fs.existsSync(path.join(workingDir, relPath));
