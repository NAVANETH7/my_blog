...
app.post('/api/posts', authenticateKey, (req: Request, res: Response) => {
  try {
...
app.delete('/api/posts/:slug', authenticateKey, (req: Request, res: Response) => {
  const { slug } = req.params;
  const filePath = path.join(postsDir, `${slug}.mdx`);
...
app.patch('/api/posts/toggle-draft', authenticateKey, (req: Request, res: Response) => {
  try {
    const { slug } = req.body;
    if (!slug) {
...
app.post('/api/upload', authenticateKey, upload.single('file'), (req: Request, res: Response) => {
  try {
    const file = req.file;
    if (!file) {
...
app.get('/api/views/:slug', async (req: Request, res: Response) => {
  try {
    const { slug } = req.params;
...
app.post('/api/views/:slug', async (req: Request, res: Response) => {
  try {
    const { slug } = req.params;
...
// --- Deploy & Git logs ---
app.get('/api/deploys', authenticateKey, async (req: Request, res: Response) => {
  try {
...
app.get('/api/git/status', authenticateKey, async (req: Request, res: Response) => {
  try {
    let workingDir = path.join(process.cwd(), '..');
    if (!fs.existsSync(path.join(workingDir, '.git')) && fs.existsSync(path.join(process.cwd(), '.git'))) {
      workingDir = process.cwd();
...
app.post('/api/deploy/trigger', authenticateKey, async (req: Request, res: Response) => {
  try {
...
// --- Settings config ---
app.get('/api/settings', authenticateKey, (req: Request, res: Response) => {
  try {
...
app.post('/api/settings/save', authenticateKey, (req: Request, res: Response) => {
  try {
...
// --- Tags Management ---
app.post('/api/tags/rename', authenticateKey, (req: Request, res: Response) => {
  try {
    ...
    for (const file of files) {
      if (file.endsWith('.mdx') || file.endsWith('.md')) {
        ...
        let tags: string[] = parsed.data.tags || [];
...
app.post('/api/tags/merge', authenticateKey, (req: Request, res: Response) => {
  try {
...
app.get('/api/media/stats', authenticateKey, (req: Request, res: Response) => {
  try {
...
app.delete('/api/media/:filename', authenticateKey, (req: Request, res: Response) => {
  try {
