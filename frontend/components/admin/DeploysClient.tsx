  ...
  useEffect(() => {
    ...
    async function fetchCommits() {
      try {
        ...
        console.error('Error fetching commits on client:', err);
      } finally {
        if (isMounted) {
          setLoading(false);
        }
        ...
        <div className="relative border-l border-slate-200 ml-4 pl-8 py-2 space-y-6">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="relative animate-pulse">
              <div className="absolute left-[-41px] top-1.5 h-6.5 w-6.5 rounded-full border border-slate-250 bg-white flex items-center justify-center" />
              <div className="glow-card-3d p-5 space-y-3.5">
                <div className="h-4.5 bg-slate-200/85 rounded-lg w-1/2" />
